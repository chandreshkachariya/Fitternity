    ...
    */
    ...
    func callApiForGetHomeWorkoutAtHome() {
        ...
        HttpClientApi.instance().callAPI(url: _api_HomeScreenAtHome, params: nil, showLoading: true,  methods: .GET, success: { (mainData) in
            ...
            Vc.getTagsFromWorkoutAtHomeVC(arrTags: self.dictStudioData.value(forKey: product_tags) as! NSArray)
...
extension WorkoutAtHomeVC: UITableViewDataSource, UITableViewDelegate {
   
    @objc func refresh(sender:AnyObject) {
       // Code to refresh table view
        refreshControl.endRefreshing()
        
        callApiForGetHomeWorkoutAtHome()
    }
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        else if section == 1 { // onepass_pre
            return 2
        }
        else if section == 2 { // personal_training
            return 0
        }
        else if section == 3 { // upcoming_classes
            return 2
        }
        else if section == 4 { // challenge
            return 1
        }
        else if section == 5 { // fit_tv
            return 1
        }
        return 0
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        else if indexPath.section == 1 { // onepass_pre
        ...
        else if indexPath.section == 2 { //personal_training
            let cell = tableView.dequeueReusableCell(withIdentifier: reuseIdentifierCampaignsTVCell, for: indexPath) as! CampaignsTVCell
